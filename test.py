...
img_zero = tab_num_sudoku1[1][0]
img_nonzero = tab_num_sudoku1[4][0]   

grad_zero = filters.sobel(img_zero)
grad_nonzero = filters.sobel(img_nonzero)

#max > 0.1 => nonzero
plt.figure(1)
plt.imshow(grad_zero)
plt.figure(2)
plt.imshow(grad_nonzero)

def img_is_empty(img):
    if (np.mean(filters.sobel(img))) < 0.01 : return True
    else : return False
    
for i in range(len(tab_num_sudoku1)):
    if (img_is_empty(tab_num_sudoku1[i][0]) == True) : 
        print("Empty")
        continue
